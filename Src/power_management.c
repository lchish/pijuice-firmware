...
void PowerManagementTask(void) {
	...
	if (MS_TIME_COUNT(powerMngmtTaskMsCounter) >= 900/*(state == STATE_LOWPOWER?2000:500)*/) {
		...
		if ( ((batteryRsoc >= wakeupOnCharge && CHARGER_IS_INPUT_PRESENT() && CHARGER_IS_BATTERY_PRESENT()) || rtcWakeupEventFlag || ioWakeupEvent) && MS_TIME_COUNT(lastHostCommandTimer) > 25000 && !delayedPowerOffCounter && MS_TIME_COUNT(lastWakeupTimer) > 30000 ) {
			if ( WakeUpHost() == 0 ) {
				rtcWakeupEventFlag = 0;
				delayedPowerOffCounter = 0;
				ioWakeupEvent = 0;
